...
public class Arene extends UnicastRemoteObject implements IAreneIHM, Runnable {
	...
	public boolean lanceAttaqueBouleDeFeu(int refRMI, int refRMIAdv) throws RemoteException {
		...
		if (personnages.get(refRMI).isActionExecutee()) {
			...
			// on teste la distance entre les personnages
			if (distance <= Constantes.DISTANCE_MIN_INTERACTION_DIST) {
				Personnage pers = (Personnage) elementFromRef(refRMI);
				Personnage persAdv = (Personnage) elementFromRef(refRMIAdv);
				...
				if (pers.estVivant() && persAdv.estVivant()) {
					console.log(Level.INFO, Constantes.nomClasse(this), 
							"Je Lance une boule de feu sur " + nomRaccourciClient(refRMIAdv));
					consoleAdv.log(Level.INFO, Constantes.nomClasse(this), 
							"Je me fait bruler par " + nomRaccourciClient(refRMI));
		...
		if (personnages.get(refRMI).isActionExecutee()) {
			...
			if (distance <= Constantes.DISTANCE_MIN_INTERACTION) {
				...
				if (pers.estVivant() && persAdv.estVivant()) {
					...
					new CoupDeHache(this, client, clientAdv).interagit();
					personnages.get(refRMI).executeAction();
	...
	public boolean Fuite(int refRMI, int refRMIaFuir) throws RemoteException {
		...
		if (client.isActionExecutee()) {
			...
			Point objectif = pMoi;
			if(pMoi.getX()-pAFuir.getX() > 0) objectif.x = Integer.min(objectif.x+4,100);
			else objectif.x = Integer.max(objectif.y-4,0);
			if(pMoi.getY()-pAFuir.getY() > 0) objectif.y = Integer.min(objectif.y+4,100);
	...
	public boolean deplaceRapidement(int refRMI, int refCible) throws RemoteException {
		...
		if (client.isActionExecutee()) {
			...
			// sinon, on tente de jouer l'interaction
			new DeplacementRapide(client, getVoisins(refRMI)).seDirigeVers(refCible);
			client.executeAction();
